    ...
    <DashboardPageShell maxWidth="wide">
        ...
        <div className="grid gap-5 2xl:grid-cols-[minmax(0,1fr)_280px]">
          <div className="min-w-0">
            ...
            <div className="mt-5 grid gap-3 sm:grid-cols-2 2xl:grid-cols-4">
              {[
                ["1W", focusedMarket.metrics.performance.oneWeek, "Mock one-week move"],
                ["1M", focusedMarket.metrics.performance.oneMonth, "Mock one-month move"],
                ["6M", focusedMarket.metrics.performance.sixMonth, "Mock six-month move"],
                ["1Y", focusedMarket.metrics.performance.oneYear, "Mock one-year move"],
              ].map(([label, value, detail]) => (
                <MetricDrilldownDrawer
                  key={label as string}
                  title={`${label} performance`}
                  description={focusedMarket.companyName}
                  trigger={
                    <button className="text-left">
                      <MetricDeltaCard
                        label={label as string}
                        value={formatPercent(value as number)}
                        detail={detail as string}
                        tone={(value as number) >= 0 ? "good" : "bad"}
                      />
                    </button>
                  }
                >
                  <div className="space-y-3 text-sm leading-6 text-muted-foreground">
                    <p>
                      {label} performance is a local mock market signal used for context only. It does not override deterministic financial risk output.
                    </p>
                    <p>
                      Current value: <span className="font-mono font-semibold text-foreground">{formatPercent(value as number)}</span>
                    </p>
                  </div>
                </MetricDrilldownDrawer>
              ))}
      ...
      <div className="grid gap-4 2xl:grid-cols-[minmax(0,1.35fr)_minmax(340px,0.65fr)]">
        ...
        <div className="space-y-4">
          ...
          <ExpandableSection title="Top movers" description="Largest absolute 1M moves among mock companies.">
          ...
          <ExpandableSection title="Momentum drivers" description="Top mock reasons generated by deterministic market rules.">
            <div className="space-y-4">
          ...
          <ExpandableSection title="Market data limits" description="Provider-ready mock scaffold only.">
